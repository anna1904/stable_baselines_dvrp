...
class DVRPEnv(gym.Env):
    ...
    def __orig_step(self, action):
        ...
        if self.clock >= self.episode_length:
            ...
            if (self.experiment_index == 1000):
                ...
                df_2.to_csv("evaluation_rs/combine_all_w_rs_6.csv", index=False)
    ...
    def __create_state(self):
        ...
        return np.array(
            [self.dr_x] + [
                self.dr_y] + self.o_x + self.o_y + statuses.flatten().tolist() + self.reward_per_order + self.o_time + distance +
